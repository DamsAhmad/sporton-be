...
const JWT_SECRET = process.env.JWT_SECRET || "Spor12345";
...
export const initiateAdmin = async (
  ...
  try {
    ...
    if (count > 0) {
      res.status(400).json({
        message:
          "We can only have 1 admin user, if you want to create new admin user, please delete the user manually from database",
      });
      return;
    }
