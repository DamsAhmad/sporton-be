...
export const getTransactions = async (
  ...
  try {
    ...
    res.status(500).json({ message: "Error fecthing transactions", error });
  }
};
...
export const updateTransaction = async (
  ...
  try {
    ...
    const exsitingTransaction = await Transaction.findById(req.params.id);
    if (!exsitingTransaction) {
      res.status(404).json({ message: "Transaction not found" });
      return;
    }

    if (status === "paid" && exsitingTransaction.status !== "paid") {
      for (const item of exsitingTransaction.purchasedItems) {
